...
contract ForTestL1OpUSDCBridgeAdapter is L1OpUSDCBridgeAdapter {
  ...
  function forTest_setIsUpgrading() external {
    isUpgrading = true;
  }
}
...
contract L1OpUSDCBridgeAdapter_Unit_MigrateToNative is Base {
   ...
   */
  function test_onlyOwner(address _newOwner, uint32 _minGasLimitReceiveOnL2, uint32 _minGasLimitSetBurnAmount) external {
    // Execute
    vm.prank(_user);
  ...
  function test_revertIfMigrationInProgress(
    ...
    adapter.forTest_setIsUpgrading();
  ...
  function test_StateOfMigration(
    ...
    assertEq(adapter.isUpgrading(), true, 'Is upgrading should be set to true');
  }
...
contract L1OpUSDCBridgeAdapter_Unit_ResumeMessaging is Base {
   ...
   */
  function test_onlyOwner(uint32 _minGasLimit) external {
    // Execute
    vm.prank(_user);
    vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, _user));
